...
if __name__ =="__main__":
	...
	for i in xrange(n_run): 
        	...
        	pairs_, classes_, cv = crossvalid.balance_data_and_get_cv(pairs, classes, n_fold, nProportion, n_subset, disjoint, n_seed )
        	auc,aupr=crossvalid.runModel( pairs_, classes_, drugFeatures, diseaseFeatures, drugFeatureNames, diseaseFeatureNames, cv, n_subset, nProportion, n_fold, model_type, model_fun, features, disjoint, n_seed, negativeSelectionScheme, output_f=output_file)
		values.append(auc)
		values2.append(aupr)
