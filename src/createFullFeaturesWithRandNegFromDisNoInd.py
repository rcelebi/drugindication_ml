...
if __name__ =="__main__":
	...
	parser.add_argument('-n', required=False, dest='negativeset', help='enter path to the file for disease in the negative set ')
	...
	#filename=sys.argv[1]
        ...
        drugs=set()
        diseasesforneg=[]
	#for line in file("../data/input-data/diseases_not_in_drugcentral.txt"):
	if disnoindfile != None:
		for line in file(disnoindfile):
			line = line.strip()
			diseasesforneg.append(line)
	...
	negative={}
	if len(diseasesforneg)==0 :
		diseasesforneg = commonDiseases
	
	for dr in commonDrugs:
		for di in diseasesforneg:
			if (dr,di) not in indClass:
				negative[(dr,di)]=0

        numIndciations=len(indClass)
        for pair in random.sample(negative, numIndciations*2):
                indClass[pair]=0

	for pair in indClass:
                (dr,di) =pair
                cls =indClass[pair]
